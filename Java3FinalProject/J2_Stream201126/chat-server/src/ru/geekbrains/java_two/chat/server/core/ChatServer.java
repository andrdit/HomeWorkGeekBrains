...
public class ChatServer implements ServerSocketThreadListener, SocketThreadListener {
    ...
    private void handleAuthClientMessage(ClientThread client, String msg) {
        ...
        switch (msgType) {
            ...
            case Protocol.USER_CHANGE_NICKNAME:
                if (arr.length != 4 || !arr[0].equals(Protocol.USER_CHANGE_NICKNAME)) {
                ...
                if (!SqlClient.setNickname(login, password, nickname)) {
                    message = "Invalid change nickname for login = " + login;
                    putLog(message);
                    sendToUser(client,Protocol.getTypeForUser(client.getNickname(), message));
                    //client.authFail();
                    return;
                }else{
                    message = "Your nickname was changed. Nickname = " + nickname + "\nYou need reconnect for update nickname.";
                    putLog("User with login = " + login + " changed nickname.");
                    sendToUser(client,Protocol.getTypeForUser(client.getNickname(), message));
                }
    ...
    private void sendToUser(ClientThread client, String msg) {
